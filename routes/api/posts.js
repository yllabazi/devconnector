...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    if(post.user.toString() !== req.user.id) {
      return res.status(401).json({msg: 'User not authorized to perform this operation!'});
    } else {
      await post.remove();
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
  try {
    ...
    // Get comment index and splice it out
